...
class Monster extends MapComponent {
    ...
    public static MapComponent[][][] updateMonster(MapComponent[][][] subMap, Tile playerTile) {
        ...
        for(Tile t : locations) {
            ...
            if (currentMonster.getMapComponentID() == RABBIT && currentMonster.getHealth() > 0) {
                int direction = (int) (Math.random() * 4);
                boolean hasMoved = false;
                if (direction == 0) {
                    try {
                        if (subMap[Map.ITEM_LAYER][row][column + 1].getWalkable() && subMap[Map.GROUND_LAYER][row][column + 1].getWalkable() && (column + 1 != ptColumn || row != ptRow))
                        {subMap[Map.ITEM_LAYER][row][column + 1] = currentMonster; hasMoved = true; }
                    } catch (Exception ex) {
                    }
                } else if (direction == 1) {
                    try {
                    ...
                    try {
                        ...
                        {subMap[Map.ITEM_LAYER][row + 1][column] = currentMonster; hasMoved = true; }
                    } catch (Exception ex) {
                    }
                } else if (direction == 3) {
                    try {
